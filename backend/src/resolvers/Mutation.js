...
const Mutations = {
  ...
  async deleteItem(parent, args, ctx, info) {
    ...
    const hasPermissons = ctx.request.user.permissions.some(permission =>
      ["ADMIN", "ITEMDELETE"].includes(permission)
    );
    if (!ownsItem && !hasPermissons) {
      throw new Error("insufficient Permissions to carry out request");
    }
    // 3. Delete it
  ...
  async signup(parent, args, ctx, info) {
    ...
    // we set the jet as a cookie on the response
  ...
  async addToCart(parent, args, ctx, info) {
    ...
    if (!userId) {
      throw new Error("You must sign in to add items to your cart");
    }
  ...
  async removeFromCart(parent, args, ctx, info) {
    ...
    if (cartItem.user.id !== ctx.request.userId) {
      throw new Error("Nope... not yours mate");
    }
